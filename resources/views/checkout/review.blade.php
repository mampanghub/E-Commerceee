                ...
                <div class="lg:col-span-2 flex flex-col gap-6">
                        ...
                        <div class="flex flex-col gap-4">
                            @foreach ($items as $item)
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex-1">
                                        <p class="font-black text-slate-900 text-sm">{{ $item['product']->nama_produk }}
                                        </p>
                                        @if ($item['variant'])
                                            <p class="text-xs text-indigo-500 font-bold">
                                                {{ $item['variant']->nama_varian }}</p>
                                        @endif
                                        <p class="text-xs text-slate-400 mt-1">{{ $item['jumlah'] }} x Rp
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-xs font-black uppercase tracking-widest text-slate-400">Alamat Pengiriman
                            </h2>
                            <button type="button"
                                onclick="openAddressModal({ action: '{{ route('addresses.store') }}', onSuccess: function() { window.location.reload(); } })"
                        ...
                        @else
                            <div class="flex flex-col gap-3" id="address-list">
                                @foreach ($addresses as $addr)
                                    <label
                                        class="address-option flex items-start gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all
                                {{ $selectedAddress && $selectedAddress->address_id == $addr->address_id ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100 hover:border-slate-200' }}">
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-1">
                                                <span
                                                    class="text-xs font-black text-slate-600 bg-slate-100 px-2 py-0.5 rounded-lg">{{ $addr->label }}</span>
                                                @if ($addr->is_default)
                                                    <span
                                                        class="text-xs font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg border border-blue-100">Utama</span>
                                                @endif
                                            </div>
                        ...
                        @if ($zone)
                            <div
                                class="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 rounded-xl border border-blue-100">
                                ...
                                <span class="text-xs text-blue-500">{{ number_format($beratGram, 0, ',', '.') }}
                                    gram</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
                        <h2 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Catatan Pesanan
                        </h2>
                        <p class="text-xs text-slate-400 mb-4">Instruksi khusus untuk penjual</p>
                        <textarea id="field-catatan" rows="3" maxlength="300" placeholder="Contoh: Tolong packing yang rapi ya."
                    ...
                    {{-- Pilih Kecepatan --}}
                    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
                        <h2 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-4">Pilih Kecepatan
                            Pengiriman</h2>
                        <div class="flex flex-col gap-3" id="shipping-options">
                            @foreach ($opsiOngkir as $opsi)
                                <label
                                    class="shipping-option-label flex items-center justify-between p-4 rounded-2xl border-2 cursor-pointer transition-all
                                {{ $selectedOptionId == $opsi['option_id'] ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100 hover:border-slate-200' }}"
                                    data-option-id="{{ $opsi['option_id'] }}" data-ongkir="{{ $opsi['ongkir'] }}">
                                    <div class="flex items-center gap-3">
                                        <input type="radio" name="shipping_option_display"
                                            value="{{ $opsi['option_id'] }}" class="accent-indigo-600"
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-400 mt-0.5">
                                                Estimasi tiba
                                                <span
                                                    class="font-bold text-slate-600">{{ $opsi['tanggal_tiba'] }}</span>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sticky top-8">
                        ...
                        <div class="flex flex-col gap-3 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold text-slate-900">Rp
                                    {{ number_format($totalProduk, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Ongkos Kirim</span>
                                <span class="font-bold text-slate-900" id="display-ongkir">Rp
                            ...
                            <div class="border-t border-slate-100 pt-3 flex justify-between">
                                ...
                                <span class="font-black text-indigo-600 text-lg" id="display-total">Rp
                        ...
                        @else
                            <form action="{{ route('checkout') }}" method="POST" class="mt-6" id="checkout-form">
                                @csrf
                                <input type="hidden" name="shipping_option_id" id="hidden-shipping-option"
                                    value="{{ $selectedOptionId }}">
                                <input type="hidden" name="address_id" id="hidden-address-id"
                        ...
                        <p class="text-center mt-4 text-[10px] text-slate-400 font-medium">🛡️ Transaksi aman &
                            terverifikasi</p>
                    </div>
                </div>
            </div>
