            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="bg-white rounded-3xl border border-slate-100 p-6">
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Status
                            Pengiriman</p>
                        <div class="flex items-center gap-3 flex-wrap">
                            @php
                                $statusConfig = [
                                    'dikemas' => ['bg-violet-100 text-violet-700', 'Siap Dikirim'],
                                    'dikirim' => ['bg-amber-100 text-amber-700', 'Sedang Dikirim'],
                                    'selesai' => ['bg-emerald-100 text-emerald-700', 'Selesai'],
                                ];
                                [$statusClass, $statusLabel] = $statusConfig[$order->status] ?? [
                    ...
                    {{-- ALAMAT TUJUAN --}}
                    <div class="bg-white rounded-3xl border border-slate-100 p-6">
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Alamat Tujuan
                        </p>
                                ...
                                <div class="flex items-center gap-4">
                                    @php
                                        $fotoVarian = $item->variant_id
                                            ...
                                            : null;
                                        $gambarDetail = $fotoVarian?->gambar ?? $item->product->primaryImage?->gambar;
                                    @endphp
                                    @if ($gambarDetail)
                                        <img src="{{ Storage::url($gambarDetail) }}"
                                            class="w-14 h-14 rounded-2xl object-cover bg-slate-100 shrink-0">
                                    @else
                                        <div
                                            class="w-14 h-14 rounded-2xl bg-slate-100 shrink-0 flex items-center justify-center">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @endif
                                        <p class="text-xs text-slate-400">{{ $item->jumlah }}x · Berat:
                                            {{ $item->variant?->berat ?? '-' }}gr</p>
                                    </div>
                    ...
                    @if ($order->status === 'selesai' && $order->foto_konfirmasi)
                        <div class="bg-white rounded-3xl border border-emerald-100 p-6">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Foto
                                Konfirmasi</p>
                ...
                <div class="space-y-5">
                    ...
                    <div class="bg-white rounded-3xl border border-slate-100 p-6">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold text-slate-800">Rp
                                    {{ number_format($order->total_harga - $order->ongkir - 2500, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                        ...
                        @if ($order->status === 'selesai')
                            <div class="mt-4 p-3 bg-emerald-50 rounded-2xl flex items-center justify-between">
                                ...
                                <p class="text-sm font-black text-emerald-700">+Rp
                        ...
                        @else
                            <div class="mt-4 p-3 bg-violet-50 rounded-2xl flex items-center justify-between">
                                ...
                                <p class="text-sm font-black text-violet-700">+Rp
                                    {{ number_format($order->ongkir, 0, ',', '.') }}</p>
                            </div>
                        @endif
                    </div>

                    {{-- Tombol ambil (dari halaman detail, sebelum diambil kurir manapun) --}}
                    @if ($order->status === 'dikemas' && is_null($order->kurir_id))
                        <div class="bg-white rounded-3xl border border-violet-100 p-6">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Ambil &
                                Kirim Paket Ini</p>
                            <p class="text-xs text-slate-400 mb-4">Setelah kamu ambil, paket ini langsung jadi tanggung
                                jawab kamu untuk dikirim.</p>
                            <form action="{{ route('kurir.orders.take', $order->order_id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    onclick="return confirm('Ambil & kirim paket #{{ $order->order_id }}?')"
                    ...
                    @if ($order->status === 'dikirim' && $order->kurir_id === Auth::id())
                        <div class="bg-white rounded-3xl border border-emerald-100 p-6">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Konfirmasi
                                Diterima</p>
                            ...
                            <form action="{{ route('kurir.orders.update-status', $order->order_id) }}" method="POST"
                                ...
                                <p class="text-[10px] text-slate-400 text-center mt-2">
                                    Ongkir <span class="font-black text-emerald-600">Rp
                                        {{ number_format($order->ongkir, 0, ',', '.') }}</span> akan masuk ke saldo
                                    kamu
                                </p>
                            </form>
                        </div>
    ...
    <div id="resi-print" class="hidden">
        <div style="font-family: monospace; width: 300px; padding: 16px; border: 2px solid #000;">
            ...
            <div style="border-top: 1px dashed #000; border-bottom: 1px dashed #000; padding: 8px 0; margin: 8px 0;">
                <div
                    style="font-size: 9px; font-weight: bold; color: #555; text-transform: uppercase; margin-bottom: 4px;">
                    Pengirim</div>
            ...
            <div style="padding: 8px 0; margin-bottom: 8px;">
                ...
                <div style="font-size: 10px; color: #333;">{{ $order->no_telp_penerima ?? $order->user->no_telp }}
                </div>
            ...
            <div style="border-top: 1px dashed #000; padding-top: 8px; font-size: 10px;">
                ...
                @if ($order->shippingOption)
                    <div style="margin-top: 2px; font-size: 9px; color: #555;">{{ $order->shippingOption->label }}
                    </div>
                @endif
