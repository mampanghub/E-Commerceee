    ...
    <div class="flex items-center gap-8 mb-10">
        <div class="text-center shrink-0">
            ...
            <div class="flex justify-center gap-0.5 my-2">
                ...
                @for ($i = 1; $i <= 5; $i++)
                    <svg class="w-5 h-5 {{ $i <= round($avg) ? 'text-indigo-500' : 'text-slate-200' }}"
        ...
        <div class="flex-1 space-y-2">
            @for ($star = 5; $star >= 1; $star--)
                ...
                <div class="flex items-center gap-3">
                    ...
                    <svg class="w-3 h-3 text-indigo-400 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 mb-8 shadow-sm">
                    ...
                    <div class="flex items-center gap-4 bg-slate-50 border border-slate-100 rounded-xl p-3 mb-5">
                        <img id="review-preview-img"
                            ...
                            class="w-14 h-14 rounded-xl object-cover bg-slate-100 shrink-0 transition-all duration-200"
                        ...
                        <div>
                            ...
                            <p id="review-preview-variant"
                               class="text-xs font-bold text-indigo-500 mt-0.5 {{ $orderPreviewData[$firstOrderId]['varian'] ? '' : 'hidden' }}">
                    ...
                    <form action="{{ route('reviews.store', $product->product_id) }}" method="POST" class="space-y-4">
                        ...
                        @if (count($orderPreviewData) > 1)
                            <div>
                                ...
                                <select id="review-order-select" name="order_id"
                                    class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-400">
                            ...
                            <div class="flex gap-1.5" id="star-selector">
                                @for ($i = 1; $i <= 5; $i++)
                                    <label class="cursor-pointer">
                                        ...
                                        <svg class="w-9 h-9 text-slate-200 hover:text-indigo-400 transition-colors star-icon"
                                            fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                        </svg>
                                    </label>
                        ...
                        <div>
                            ...
                            <textarea name="komentar" rows="3"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-400 transition-all"
                                placeholder="Ceritakan pengalamanmu dengan produk ini..."></textarea>
                        </div>

                        <button type="submit"
                            class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl transition-all text-sm active:scale-95">
                            Kirim Ulasan
                        </button>
                    </form>
                </div>
            @else
                <div class="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3 mb-6 text-sm text-emerald-700 flex items-center gap-2 font-medium">
    ...
    document.querySelectorAll('#star-selector label').forEach((label, idx) => {
        label.addEventListener('click', () => {
            ...
            document.querySelectorAll('.star-icon').forEach((icon, i) => {
                icon.classList.toggle('text-indigo-500', i < val);
