...
<div id="address-modal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    ...
    <div class="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6 my-8">
        ...
        <form id="addr-form" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Label</label>
                <input type="text" name="label" id="af-label" placeholder="Rumah / Kantor / dll"
                    class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
            </div>

            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Nama Penerima</label>
                    <input type="text" name="nama_penerima" id="af-nama"
                        class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">No. Telepon</label>
            ...
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Alamat</label>
                <textarea name="alamat" id="af-alamat" rows="2"
                    class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none" required></textarea>
            </div>

            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Provinsi</label>
            ...
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Kota / Kabupaten</label>
            ...
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Kecamatan</label>
            ...
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5">Kelurahan</label>
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_default" id="af-default" value="1" class="w-4 h-4 accent-blue-600">
...
function openAddressModal(opts = {}) {
    const { title = 'Tambah Alamat', action, editId, data } = opts;
    ...
    ['af-city','af-district','af-village'].forEach(id => {
        ...
        const label = {
            'af-city': 'Kota', 'af-district': 'Kecamatan', 'af-village': 'Kelurahan'
        }[id];
...
function openModal()               { openAddressModal(); }
function openEditModal(id, data)   { openAddressModal({ title: 'Edit Alamat', action: `/profile/addresses/${id}`, editId: id, data }); }
