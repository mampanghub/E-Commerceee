<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
            <div class="mb-6 bg-green-50 border border-green-200 text-green-700 px-5 py-4 rounded-2xl font-bold text-sm">
                ✅ {{ session('success') }}
            </div>
            @endif
            ...
            <div class="bg-white shadow-xl sm:rounded-[2rem] border border-gray-100 p-8">
                <form method="post" action="{{ route('profile.update') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        @if(auth()->user()->store)
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-blue-50 border border-blue-100 p-5 rounded-2xl">
                            <div>
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Toko</span>
                                <p class="font-black text-gray-800 mt-1">{{ auth()->user()->store->nama_toko }}</p>
                            </div>
                            <div>
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Status Alamat Toko</span>
                                @if(auth()->user()->store->province_id)
                                <p class="text-xs text-green-600 font-bold mt-2">✅ Alamat toko sudah tersimpan</p>
                                @else
                                <p class="text-xs text-red-500 font-bold mt-2">⚠ Belum diisi — ongkir tidak akan terhitung!</p>
                                @endif
                            </div>
                        </div>
                        @else
                        <div class="bg-red-50 border border-red-100 p-5 rounded-2xl">
                            <p class="text-sm text-red-500 font-bold">⚠ Toko belum terdaftar! Jalankan StoreSeeder dulu.</p>
                        </div>
                        @endif
                    </div>
                    @endif
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-2xl p-5 flex items-center justify-between">
                        <div>
                            <p class="text-xs font-black uppercase tracking-widest text-blue-400 mb-1">Saldo Refund</p>
                            <p class="text-2xl font-black text-blue-700">Rp {{ number_format(auth()->user()->saldo, 0, ',', '.') }}</p>
